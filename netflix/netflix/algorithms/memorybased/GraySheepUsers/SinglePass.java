...
public class SinglePass
{
	...
	public int getSpheresIDByUID(int uid)    
	{
		...
		//If not similar to any sphere then find the sphere with whome it is most similar..... 
		if (insphere == false) 
		{ 
			for(int i = 0; i < sphereCentroids.size(); i++)        
			{
			...
			double cliqueAverage) 		//golbal average in the database    
	{
		...
		for(int i=0; i < dataset.size(); i++)           
		{
			...
			if(simVersion==1)
				totalError +=   sphereCentroids.get(tempCluster).distanceWithDefaultSphere(point, cliqueAverage, helper);
			else if(simVersion==2)
				totalError +=   sphereCentroids.get(tempCluster).distanceWithoutDefaultSphere(point, cliqueAverage, helper);
			else if(simVersion==3)
				totalError +=   sphereCentroids.get(tempCluster).distanceWithDefaultVSSphere(point, cliqueAverage, helper);
			else if(simVersion==4)
				totalError +=  sphereCentroids.get(tempCluster).distanceWithoutDefaultVSSphere(point, cliqueAverage, helper);
			else if(simVersion==5)
				totalError +=   sphereCentroids.get(tempCluster).distanceWithPCCSphere(point, i, helper);   	 
			else if(simVersion==6)
				totalError +=   sphereCentroids.get(tempCluster).distanceWithVSSphere(point, i, helper);


		} 
		System.out.println( "Similarity = " + totalError);
			...
			double cliqueAverage)     
	{
		...
		{         
			//     if(simVersion==1)
			distance = centroids.get(i).distanceWithDefaultSphere(uid, cliqueAverage, helper);
			//        	else if(simVersion==2)
			//        		distance = centroids.get(i).distanceWithoutDefault(uid, cliqueAverage, helper);
			//        	else if(simVersion==3)
			//        		distance = centroids.get(i).distanceWithDefaultVS(uid, cliqueAverage, helper);
